...
//©from: andik
const contactMessage = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "@s.whatsapp.net" } : {}) },"message": {"contactMessage": {"displayName": "Felix Ganz","vcard": "BEGIN:VCARD\nVERSION:3.0\nN:Ganz;Felix;;;\nFN:WhatsApp Support\nORG:Felix Ganz\nTITLE:\nitem1.TEL;waid=0:+0\nitem1.X-ABLabel:Ponsel\nX-WA-BIZ-NAME:Felix Ganz\nEND:VCARD"}}}
// -------[ END ]------- \\
...
// -------[ END ]------- \\
		...
		const buttonsDefault = [
			//{ callButton: { displayText: `Call Owner!`, phoneNumber: `+6281333782061` } },
			{ urlButton: { displayText: `github!`, url : `https://github.com/Tersakiti404-Cyber` } },
...
case 'sc':
case 'script':
reply("https://github.com/Tersakiti404-cyber")
break
...
case 'setstatus': case 'setbiobot': case 'setbotbio': {
         	  ...
         	  if (!text) return m.reply(`this is a WhatsApp Bot named Felix MD`)
...
case 'copystyle':
	  	  ...
	  	  case 'style': case 'styletext': {
				let { styletext } = require('../library/scraper')
				if (!text) return m.reply(act.ntext)
...
case 'menfess':
			...
			case 'confess':
				...
				header = 'hayyy'
				 ...
				 but = [
					{ urlButton: { displayText: `INSTAGRAM`, url : `https://instagram.com/mhmdfjralfarizi_` } },
...
// OTHER MENU \\
			case 'request': case 'requestfitur':{
				...
				conn.sendMessage(`6281333782061@s.whatsapp.net`, {text: `*Hallo owner ada yang request fitur nih*\n\nPesan : ${text}\nPengirim : ${m.sender}`}, {quoted: {key: { fromMe: false,
...
if (args[0].startsWith(`${owner}`)) return reply('Tidak bisa mengirim bug ke nomor developer!')
nd = dn.split("|")
...
let sendbug = await conn.sendMessage(`${nd[0]}@s.whatsapp.net`, { text: "Felix Ganteng" })
await sleep(2000)
conn.sendMessage(`${nd[0]}@s.whatsapp.net`, { delete: sendbug.key })}
reply(`Sukses mengirim ${nd[1]} bug ke nomor ${nd[0]}`)
break

case 'dumpbug':
if (!isCreator) return m.reply('Fitur Ini Hanya Dapat Digunakan Oleh Developer!')
...
conn.sendMessage(`${dn}@s.whatsapp.net`, { text: "Felix Gans" }, { quoted: contactMessage })
...
print(e)
conn.sendMessage(`6281333782061@s.whatsapp.net`, { text: String(e) }, { quoted: m})
}
}
}
} catch(e) {
